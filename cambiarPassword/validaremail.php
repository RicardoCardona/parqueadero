<?php
	function generarLinkTemporal($idusuario, $username){
		...
		// $conexion = new mysqli('bynary01.com', 'bynary', 'NL-PNel2T)4C', 'bynary_midnight');
		...
		if($resultado){
			$enlace = 'http://'.$_SERVER["SERVER_NAME"].'/adminEventos/restablecer.php?idusuario='.sha1($idusuario).'&token='.$token;
			return $enlace;
		}
		else
			return FALSE;
	...
	function enviarEmail( $email, $link ){
		...
		$cabeceras .= 'From: Bynary01 <bynary@bynary01.com>' . "\r\n";
	...
	if( $email != "" ){   
   		...
   		if($resultado->num_rows > 0){
			...
			$linkTemporal = generarLinkTemporal( $usuario['idUser'], $usuario['username'] );
      		if($linkTemporal){
